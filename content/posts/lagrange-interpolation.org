...
\] Fitting a polynomial: Lagrange interpolation produces a *perfect
fit*, i.e. \[
\begin{align*}
...
\end{align*}
\] The advantage of Lagrange interpolation is that no training is
required. All we need to do is to write down the \(L\).
